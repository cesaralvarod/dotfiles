...
function compile(document) {
  return new Promise((resolve) => {
    const child = (0, import_child_process.spawn)("solc", [
      "--standard-json",
      "--base-path",
      rootPath,
      "--include-path",
    ...
    child.stdout.on("end", () => {
      ...
      resolve(Object.values(sources).map((i) => {
        const ast = i.ast;
    ...
    child.on("error", ({ message }) => {
      connection?.sendRequest(import_node.ShowMessageRequest.type, {
        type: import_node.MessageType.Error,
        message
      });
    });
    const filename = getFilename(document);
    const remappings = Object.keys(options.remapping).map((key) => `${key}=${options.remapping[key]}`);
...
function showErrors(document, errors) {
  errors = errors.filter((i) => document.uri.indexOf(i.sourceLocation.file) != -1);
  const diagnostics = [];
...
async function onDefinition({
  ...
  if (node.nodeType == "ImportDirective") {
    const uri2 = getAbsoluteUri(node.absolutePath);
...
// src/parse.ts
function parseAST(node, root, identifiers, definitions, scopes, nodes, nodeMap) {
  node.root = root;
...
var Solidity = class {
  ...
  constructor(document, sources) {
    ...
    for (const root of sources) {
      ...
      parseAST(root, root, this.identifiers, this.definitions, this.scopes, this.nodes.get(uri), this.nodeMap);
    }
  }
...
var rootPath = (0, import_fs3.realpathSync)(".");
var connection;
var documents;
...
function createServer(input, output) {
  ...
  documents.onDidChangeContent(async ({ document }) => {
    ...
    if (result.length) {
      ...
      connection.sendNotification(import_node2.DidOpenTextDocumentNotification.type, {
        textDocument: import_node2.TextDocumentItem.create(document.uri, "", 0, "")
      });
    }
    require("prettier");
  });
...
function loadingRemappingsFile() {
  const remappingsFile = (0, import_path3.join)(rootPath, "remappings.txt");
  (0, import_fs3.readFile)(remappingsFile, null, (err, data) => {
    if (!err) {
      data.toString().split("\n").forEach((line) => {
